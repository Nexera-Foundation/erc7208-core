...
/**
 * @title Callback Processor DataObject (Upgradeable)
 * @notice Upgradeable mixin for DataObjects that need to trigger external callback handlers after
 * write operations. Handlers are registered per DataPoint with a bitmask filter, so each handler
 * only receives callbacks for the tasks it is interested in. The extending contract calls
 * `_processCallbacks()` from its `_dispatchWrite()` after performing the write logic.
 *
