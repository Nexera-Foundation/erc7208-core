 ...
 * @dev In order to prevent locked-ether in allocate(), it is recommended to add a method to withdraw
 *      native coin balance.
 */
interface IDataPointRegistry {
    ...
    /**
     * @dev Error thrown when caller is not the owner of the DataPoint
    ...
    /**
     * @notice Grant permission to grant/revoke other roles on the DataPoint inside an Data Index Implementation
    ...
    /**
     * @notice Revoke permission to grant/revoke other roles on the DataPoint inside an Data Index Implementation
     * @param dp DataPoint identifier
